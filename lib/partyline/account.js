        ...
        global.db.query(query, [], function (error, results, fields) {
            if (error) {
                ...
                for (key in results) {
                    ...
                    // TODO: this should probably be sent as one large chunk to prevent 
                    //       new messages coming in while it is being populated
                    let msgName = message.highlight(result.username, 'name', 'r') + ' says'
                    let content = message.highlight(result.content, 'message')
